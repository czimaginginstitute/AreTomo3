...
MMD::CStackShift* CInterpolateShift::DoIt
...
{	MMD::CFmGroupParam* pFmGrpParam = 
	   MMD::CFmGroupParam::GetInstance(iNthGpu, bPatch);
	MMD::CStackShift* pStackShift = new MMD::CStackShift;
	pStackShift->Setup(pFmGrpParam->m_iNumIntFms);
	this->DoIt(pGroupShift, pStackShift, iNthGpu, bPatch, bNearest);
	return pStackShift;
}
...
{	MMD::CFmGroupParam* pFmGrpParam =
	...
	MMD::CFmIntParam* pFmIntParam =
	   MMD::CFmIntParam::GetInstance(iNthGpu);
	//-----------------
	...
	{	float afShift[2] = {0.0f};
		...
		{	int iGpStart = pFmGrpParam->GetGroupStart(g);
			int iGpSize = pFmGrpParam->GetGroupSize(g);
	...
	for(int i=0; i<m_iNumGroups; i++)
	{	pfGroupCents[i] = pFmGrpParam->GetGroupCenter(i);
	}
	//---------------------------------------------------------
